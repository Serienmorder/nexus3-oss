...
options:
  ...
  repository_name:
    description:
      - Name of the repository in Nexus
    type: str
  return_content:
    description:
...
def build_URL(baseurl, endpoint, repository, asset_id):
    if asset_id is not None:
        return baseurl + '/service/rest/' + endpoint + '/assets/' + asset_id

    if repository is not None:
        return baseurl + '/service/rest/' + endpoint + '/assets?repository=' + repository
...
def main():
    ...
    repo_name = module.params['repository_name']
    assetID = module.params['asset_id']
    socket_timeout = 30

    dict_headers = module.params['headers']

    url = build_URL(base_url, endpoint, repo_name, assetID)
    if body_format == 'json':
